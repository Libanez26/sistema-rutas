...
st.set_page_config(page_title="Gestión de Rutas Avanzada", layout="wide")

# ==========================================
# GESTIÓN DE ESTADO GLOBAL Y COOKIES
# ==========================================
if "usuario" not in st.session_state:
    st.session_state["usuario"] = None

if "dispositivo_confiable" not in st.session_state:
    st.session_state["dispositivo_confiable"] = False

# Configurar API de Gemini con soporte multinivel y reintentos
...
if st.session_state["usuario"] is None:
    ...
    with tab_login:
        ...
        recuerdame = st.checkbox("Confiar en este dispositivo (Mantener sesión activa)")
        ...
        if st.button("Ingresar", type="primary"):
            if supabase:
                try:
                    ...
                    st.session_state["dispositivo_confiable"] = recuerdame
    ...
    if st.session_state["dispositivo_confiable"]:
        st.sidebar.caption("🔒 Dispositivo seguro verificado")
...
with tab_general:
    ...
    st.markdown("Sube tu archivo Excel o PDF para cargar información completa de forma automática.")
    ...
    if uploaded_file and st.button("Procesar y Organizar con IA"):
        with st.spinner("Leyendo documento y estructurando todos los datos con Gemini..."):
            try:
                if uploaded_file.name.endswith(".xlsx"):
                    ...
                    for col in df_excel.select_dtypes(include=["object"]).columns:
                        df_excel[col] = df_excel[col].astype(str).str.strip().replace({"nan": "", "None": ""})
                    ...
                    for col_target in columnas_clientes:
                        ...
                        if col_target in df_excel.columns:
                            ...
                            for col_excel in df_excel.columns:
                                if col_excel.lower() == col_target.lower():
                    ...
                    st.success("¡Archivo Excel cargado con éxito en la vista previa!")
                else:
                    ...
                    st.success("¡Datos del PDF extraídos con éxito!")
            except Exception as e:
    ...
    st.markdown("---")
    st.subheader("Opciones de Visualización y Descarga Interactiva")
...
vendedores_disponibles = sorted(list(set(df_seguimiento["Vendedor"].dropna().astype(str)) - {"\x00"}))
...
if vendedores_disponibles:
    ...
    with col_f4:
        dia_seleccionado = st.selectbox("Seleccionar Día de Visita", ["Lunes", "Martes", "Miércoles", "Jueves", "Viernes", "Sábado", "Domingo"], key="filtro_dia_ruta")
    ...
    if not df_filtrado.empty:
        cols_a_mostrar = ["Nro", "Cliente", "Ubicacion", "Nro de Ruta (Ventas)", "Visita_S1", "Pedido_S1", "Motivo_Pedido_S1"]
        df_view = df_filtrado[cols_a_mostrar].copy()

        for col_bool in ["Visita_S1", "Pedido_S1"]:
            df_view[col_bool] = df_view[col_bool].apply(lambda x: True if str(x).strip().lower() in ["sí", "si", "true", "1"] else False)
        ...
        with st.form("form_ruta_vendedores"):
            ...
            if st.form_submit_button("💾 Guardar y Desplazar Historial", type="primary", use_container_width=True):
...
datos_vista_despacho = []
    ...
    for _, r in df_despachos.iterrows():
        ...
        if dia_v_s1 and str(dia_v_s1).strip() not in ["", "nan", "None", "No asignado"]:
            ...
            for d_item in [d.strip() for d in str(desp_s1).split(",")]:
                if f"({semana_filtro_esp})" in d_item:
                    datos_vista_despacho.append({
                        "Cliente": cliente_val,
                        "Ubicación": ubicacion_val,
                        "Tiempo de Despacho": tiempo_desp,
                        "Día de Despacho": d_item
                    })

    if datos_vista_despacho:
        st.dataframe(pd.DataFrame(datos_vista_despacho), use_container_width=True, hide_index=True)
    else:
        st.info(f"No hay despachos programados para la **{semana_filtro_esp}**.")
